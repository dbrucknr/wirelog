...
| phase | expected impact | mechanism |
|---|---|---|
| Phase 4 — compile-time level filtering | disabled path returns to ~0 ns | `#[cfg(feature)]` eliminates call sites entirely |
| Phase 5 — `NonBlocking<W>` | active path drops significantly | `mpsc` channel removes mutex from the hot path |
